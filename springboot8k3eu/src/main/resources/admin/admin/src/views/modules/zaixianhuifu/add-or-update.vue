<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="回复时间" prop="huifushijian">
            <el-date-picker
        ...
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.huifushijian" label="回复时间" prop="huifushijian">
              <el-input v-model="ruleForm.huifushijian" 
                placeholder="回复时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="回复内容" prop="huifuneirong">
                <editor 
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.huifuneirong" label="回复内容" prop="huifuneirong">
                    <span v-html="ruleForm.huifuneirong"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
	...
	this.ruleForm.huifuneirong = this.ruleForm.huifuneirong.replace(reg,'../../../springboot8k3eu/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `zaixianhuifu/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.zaixianhuifuCrossAddOrUpdateFlag = false;
						 this.parent.search();
