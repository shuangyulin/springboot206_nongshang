<template>
  <div class="main-content">
        ...
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
            ...
            <el-table-column width="300" :align="contents.tableAlign" 
               :header-align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('dingdanxinxi','交货') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="dingdanzhuizongCrossAddOrUpdateHandler(scope.row,'cross','','dingdanzhuangtai','已进行交货！','已交货,待交货'.split(',')[0])">{{ contents.tableBtnFont == 1?'交货':'' }}</el-button>
                <el-button v-if="isAuth('dingdanxinxi','交货') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="dingdanzhuizongCrossAddOrUpdateHandler(scope.row,'cross','','dingdanzhuangtai','已进行交货！','已交货,待交货'.split(',')[0])">{{ contents.tableBtnFont == 1?'交货':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('dingdanxinxi','交货') && contents.tableBtnIcon == 0" type="success" size="mini" @click="dingdanzhuizongCrossAddOrUpdateHandler(scope.row,'cross','','dingdanzhuangtai','已进行交货！','已交货,待交货'.split(',')[0])">{{ contents.tableBtnFont == 1?'交货':'' }}</el-button>
    ...
    <dingdanzhuizong-cross-add-or-update v-if="dingdanzhuizongCrossAddOrUpdateFlag" :parent="this" ref="dingdanzhuizongCrossaddOrUpdate"></dingdanzhuizong-cross-add-or-update>
...
import dingdanzhuizongCrossAddOrUpdate from "../dingdanzhuizong/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      dingdanzhuizongCrossAddOrUpdateFlag: false,
  ...
  components: {
    AddOrUpdate,
    dingdanzhuizongCrossAddOrUpdate,
  },
  methods: {
    ...
    dingdanzhuizongCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.dingdanzhuizongCrossAddOrUpdateFlag = true;
		...
		for (var o in obj){
		  if(o==statusColumnName && obj[o]==statusColumnValue){
			...
			this.getDataList();
		      ...
		      this.showFlag = true;
		      this.dingdanzhuizongCrossAddOrUpdateFlag = false;
			return;
		  }
		}
	}
      this.$nextTick(() => {
      this.$refs.dingdanzhuizongCrossaddOrUpdate.init(row.id,type);
      });
    },
    payHandler(row){
